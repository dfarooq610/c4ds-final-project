import AcreInteractive from "../components/Chapter2/AcreInteractive.jsx";
export default function Chapter2() {
  return (
    <div className="ChapterContainer LightMode">
      <div className="ChapterContainer2">
        <header>
          <h2 className="LightMode">Impacts</h2>
          <h3 className="Chapter2Subtitle">
            Measuring the impact of wildfires in the United States over the past
            decade
          </h3>
        </header>
          <div className="Row">
            <p>
              The Carlton Complex fire of 2014 marked a significant turning point in
              the United States' experience with wildfires. As the largest wildfire
              in Washington State's history, the Carlton Complex consumed over
              250,000 acres, destroyed more than 300 homes, and cost millions of
              dollars in damages. The devastation left behind by the firestorm
              served as a stark reminder of the potential danger and far-reaching
              consequences of wildfires in an era of climate change.
            </p>
            <AcreInteractive />
          </div>
          <article>
          <div>
            <p>
              The Carlton Complex fire significantly impacted the environment and
              public life during and after the event.
            </p>
            <p>
              The Carlton Complex fire destroyed large areas of vegetation, which
              had a significant ecological impact. The vegetation plays a critical
              role in the ecosystem by providing a habitat for wildlife, reducing
              soil erosion, and improving water quality.
            </p>
            <p>
              The loss of vegetation can have far-reaching consequences on the
              ecosystem, including increased soil erosion and reduced water quality.
            </p>
            <p>
              "The fire also destroyed large areas of forest, which will take years
              to regenerate." Debbie Grable said. "The forest is a vital ecosystem
              component and provides essential benefits such as carbon sequestration
              and habitat for wildlife."
            </p>
            <p>
              Debbie Grable, a 70-year-old resident of Northern California, vividly
              remembers the terrifying ordeal of the Carlton Complex fire in 2014.
            </p>
            <p>
              Debbie Grable and her husband, [first name, last name], shared that
              the memory of the wildfire still sends shivers down her spine and
              continues to haunt her to this day. The fire, which ravaged large
              parts of the Methow Valley, was one of the most destructive wildfires
              in the history of the state, leaving a trail of destruction in its
              wake.
            </p>
            <p>
              For her, the experience of living through the Carlton Complex fire was
              nothing short of harrowing. The fast-moving flames, strong winds, and
              dry conditions made for a volatile and unpredictable situation that
              left many residents feeling helpless and afraid.
            </p>
            <p>
              In the aftermath of the fire, Debbie Grable and her fellow community
              members had to band together to pick up the pieces and rebuild their
              homes and lives from scratch.
            </p>
            <p>
              She said the loss of forest cover could contribute to climate change
              and threaten the survival of many wildlife species.
            </p>
            <p>
              "Also, smoke and ash filled the air during the fire, resulting in a
              decline in air quality," she added. "The smoke and ash contained
              harmful chemicals, which can cause respiratory problems in humans and
              animals."
            </p>
            <p>
              Debbie Grable said people with pre-existing respiratory conditions
              such as asthma and chronic obstructive pulmonary disease were
              particularly vulnerable.
            </p>
            <p>
              "Even after the fire was extinguished, the ash and debris from the
              fire continued to pose a threat to public health," she said.
            </p>
            <p>
              Furthermore, The Carlton Complex fire impacts the water quality in the
              surrounding area.
            </p>
            <p>
              "The fire destroyed vegetation, which reduced the ability of the land
              to absorb rainfall. This increased the risk of flooding and soil
              erosion," [first name] Grable said.
            </p>
            <p>
              He said the ash and debris from the fire could also contaminate water
              sources, affecting the quality of drinking water.
            </p>
            <p>
              "I have to say that The Carlton Complex fire had a profound impact on
              the public's life, and the destruction of homes and property led to
              significant economic losses for individuals and the community," [first
              name] Grable said.
            </p>
            <p>
              He mentioned that the fire also displaced many residents, causing
              emotional and psychological trauma.
            </p>
            <p>
              "The smoke and ash from the fire directly impacted the health of
              individuals, particularly those with pre-existing respiratory
              conditions," he said.
            </p>
            <p>
              Besides, Debbie and [first name] Grable highlighted that The Carlton
              Complex fire caused significant economic losses to individuals,
              businesses and the community.
            </p>
            <p>
              "The destruction of homes and property resulted in millions of dollars
              in damages," Debbie Grable said. "The economic impact of the fire was
              felt across various industries, including agriculture, forestry, and
              tourism."
            </p>
            <p>
              She said the loss of forest cover could also have long-term economic
              consequences, such as reduced carbon sequestration and decreased
              wildlife habitat.
            </p>
            <p>
              The Carlton Complex fire had a far-reaching impact on the environment
              and the public's life. The destruction of homes and property, the loss
              of vegetation, and the impact on water and air quality are just some
              of the ways in which the fire affected the community.
            </p>
          </div>
        </article>
      </div>
    </div>
  );
}
